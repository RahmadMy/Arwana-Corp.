...
const AdminUsers = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEdit && !formData.password) return setError('Password is required')
    ...
    try {
      ...
      // password only sent if provided
  ...
  const openDeleteModal = (id) => {
    if (currentUserId === id) return // Double check
    setDeleteId(id)
            ...
            >
              ...
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <span className={`inline-block px-2 py-1 mb-2 text-xs font-bold tracking-wider rounded-sm uppercase ${item.role === 'admin' ? 'bg-orange-500/20 text-orange-400 border border-orange-500/50' : 'bg-gray-800 text-gray-400 border border-gray-700'
                      }`}>
                  ...
                  <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center text-gray-500 group-hover:bg-yellow-400 group-hover:text-black transition-colors">
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-gray-800 mt-auto">
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-lg transition-colors ${currentUserId === item.id
                        ? 'text-gray-600 bg-gray-800/50 cursor-not-allowed'
                        : 'text-gray-400 hover:text-red-500 hover:bg-red-500/10'
                      }`}
                    title={currentUserId === item.id ? "Cannot delete own account" : "Delete User"}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-black border border-gray-700 rounded-xl w-full max-w-lg p-6">
            ...
            {error && (
              <div className="mb-4 text-red-500 text-sm bg-red-500/10 p-3 rounded border border-red-500/20">
                {error}
              </div>
            )}
            ...
            >
              <div>
                <label className="text-gray-500 text-sm mb-1 block">Name</label>
                <input
                  name="name"
                  value={formData.name}
              ...
              <div>
                <label className="text-gray-500 text-sm mb-1 block">Role</label>
                <select
                  name="role"
                  value={formData.role}
                  onChange={handleChange}
                  className="w-full bg-black border border-gray-600 text-white px-4 py-2 rounded focus:border-yellow-400 focus:outline-none transition-colors"
                >
                  <option value="petugas">Staff (Worker)</option>
              ...
              <div>
                <label className="text-gray-500 text-sm mb-1 block">
                  Password {isEdit && <span className="text-gray-500 font-normal">(Leave blank to keep unchanged)</span>}
                </label>
                <input
                  name="password"
                  type="password"
